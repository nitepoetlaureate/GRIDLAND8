"""Normalized discovery models."""
from __future__ import annotations

from typing import Literal

from pydantic import BaseModel, Field, field_validator


SourceName = Literal["osm", "fcc", "mapillary", "crtsh"]


class CameraResult(BaseModel):
    """A single normalized camera or surveillance device.

    The dict serialization of this model is what /api/discover returns and what
    `compliance.guardrails.is_compliant_camera` checks. Any field rename here
    must also update shared/constants.py.
    """
    id: str
    lat: float = Field(..., ge=-90.0, le=90.0)
    lon: float = Field(..., ge=-180.0, le=180.0)
    source: SourceName
    label: str
    url: str = ""
    thumbnail_url: str | None = None
    blur_required: bool = True
    data_age_s: int = 0
    fetched_at: str

    @field_validator("url")
    @classmethod
    def url_no_creds(cls, v: str) -> str:
        if "@" in v.split("://", 1)[-1].split("/", 1)[0]:
            raise ValueError("url contains embedded credentials")
        return v


class DiscoveryResponse(BaseModel):
    query: dict
    results: list[CameraResult]
    fetched_at: str
    counts_by_source: dict[str, int]
